...
const castToString = val => {
  ...
  throw new this.CastError(String, val);
};

const castToNumber = val => {
  if(isNumber(val)) return val;
  const number = Number(val);
  if(isNaN(number)) throw new this.CastError(Number, val);
  return number;
};
...
const castToBoolean = val => {
  ...
  throw new this.CastError(Boolean, val);
};
...
const castToArray = caster => val => {
  try {
    ...
    throw new this.CastError(Array, val);
  }
};
